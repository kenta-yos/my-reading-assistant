...

{
  ...
  "prerequisites": {
    ...
    "uniqueness": "同じテーマを扱う他の有名な本（2〜3冊を具体的に書名で挙げる）と比較して、この本ならではの独自性・切り口・価値を説明する（4〜6文）。『他の本は○○だが、この本は△△』という対比構造で書くこと",
    ...
    "selfCheckQuiz": [
      {"question": "この本を読む準備ができているか確認するための問い（Yes/Noまたは短答で答えられる形式）", "answer": "模範解答（1〜2文）", "topic": "この問いが確認している前提知識の領域名"}
    ],
...
difficultyDimensions は5軸すべてを1〜5で返すこと：
- vocabulary: 専門用語の難しさ（1=日常語のみ、5=専門辞書が必要）
- concepts: 概念・理論の抽象度（1=具体的、5=高度に抽象的）
- formality: 数式・論理記号・形式的記法の多さ（1=なし、5=数式中心）
- volume: 分量・情報密度（1=薄い/平易な語り口、5=大部/情報が凝縮）
- culturalContext: 特定の文化的・歴史的知識の必要度（1=普遍的、5=特定文化圏の深い知識が必要）

selfCheckQuiz は5問。この本を読む前提知識があるかを読者自身がチェックするための問い。
各問いは prerequisiteKnowledge や terminology の中で特に重要なものに対応させ、topic でどの領域の確認かを明示すること。
問いは「○○とは何か？」「○○と△△の違いは？」「○○が起きた背景は？」のような具体的な形式にすること。
