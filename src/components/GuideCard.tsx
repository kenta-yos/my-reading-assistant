...
export default function GuideCard({
  ...
  if (compact) {
    return (
      <Link href={`/guides/${id}`} className="group block h-full">
        <article className="h-full overflow-hidden rounded-xl border border-stone-200 bg-white transition group-hover:border-indigo-300 group-hover:shadow-sm dark:border-stone-700 dark:bg-stone-900">
          <div className="h-0.5 bg-indigo-400 opacity-0 transition group-hover:opacity-100" />
          <div className="p-4">
            <p className="mb-2 text-xs text-stone-500">
              {inputType === 'URL' ? 'URL' : '書籍'} ·{' '}
              {new Date(createdAt).toLocaleDateString('ja-JP', { month: 'numeric', day: 'numeric' })}
            </p>
